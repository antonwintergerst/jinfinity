<?php 
/**
 * @version     $Id: jiimagegallery.php 090 2014-12-18 16:33:00Z Anton Wintergerst $
 * @package     JiImageGallery Content Plugin for Joomla 1.7+
...
// Load Helper Class
require_once(JPATH_SITE.'/plugins/content/jiimagegallery/helper.php');

class plgContentJiImageGallery extends JPlugin {
    ...
    public function onContentPrepare($context, &$article, &$params, $limitstart = 0)
    {
        ...
        if($app->isSite()) {
            $this->imageGallery($context, $article);
        }
    }

    private function imageGallery($context, $item)
    {
        ...
        if($changed) {
            ...
            $document->addStyleSheet(JURI::root().'media/jiimagegallery/css/gallery.css');
        }
    }
    ...
    public function addHTML($matches)
    {
        ...
        $helper = new plgJiImageGalleryHelper();
        $helper->debug = $params->get('debug', 0);
        // Just display the file gallery
    ...
    private function getParams($pluginType='content', $pluginName='jiimagegallery')
    {
