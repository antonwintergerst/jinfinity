<?php
/**
 * @version     $Id: radio.php 093 2014-12-24 10:17:00Z Anton Wintergerst $
...
class JiCustomFieldRadio extends JiCustomField {
    /**
    ...
    public function renderInputScript() {
        ...
        <script type="text/javascript">
            (function(jQuery){
                var JiFieldRadio = function(container, data)
                {
                    var self = this;
                    // Set Default Data
                    this.data = {
                            ...
                            jQuery.ajax({url:'index.php?option=com_jicustomfields&task=fields.renderinputoption&format=ajax',
                                ...
                                data:{
                                    ...
                                    if(response!=null) {
                                        ...
                                        // Insert new radio option
                                        var html = '\<div class="radiobox"><input type="radio" id="fieldvalue'+self.data.id+optioncount+'" name="jifields['+self.data.id+'][value]" class="radiobtn" value="'+value+'"/>\<label for="fieldvalue'+self.data.id+optioncount+'" class="radiolabel">'+label+'\</label>\</div>';
                                        jQuery('#jifields_'+self.data.id).append(html);
                ...
                jQuery.fn.jifieldradio = function(data) {
                    var element = jQuery(this);
                    if(element.data('jifieldradio')) return element.data('jifieldradio');
                    var jifieldradio = new JiFieldRadio(this, data);
                    element.data('jifieldradio', jifieldradio);
                    return jifieldradio;
                };
            })(jQuery);
        </script>
    ...
    public function renderInput() {
        ...
        ob_start(); ?>
        <div class="select input ji<?php echo $this->type; ?>">
            <div id="<?php echo $this->get('inputid'); ?>" class="radioparent">
                <?php $count = 0;
                foreach($options as $value=>$label):
                    $checked = ($this->get('value')==$value)? ' checked="checked"':''; ?>
                    <div class="radiobox">
                        <input type="radio" id="fieldvalue<?php echo $this->get('id').$count; ?>" name="<?php echo $this->get('inputname'); ?>[value]" class="radiobtn" value="<?php echo $value; ?>" <?php echo $checked; ?>/>
                        <label for="fieldvalue<?php echo $this->get('id').$count; ?>" class="radiolabel"><?php echo $label; ?></label>
                    </div>
                    <?php $count++;
                endforeach; ?>
            </div>
    ...
    /**
     * Returns options for this field
     * @return array
     */
    ...
    public function renderInputParams() {
        ...
        <div class="jitable optionstable">
            <ul class="jitrow row-fluid nodrop">
                <li class="jitd span12 header"><?php echo JText::_('JICUSTOMFIELDS_RADIO').' '.JText::_('JICUSTOMFIELDS_FIELDPARAMS'); ?></li>
            </ul>
            <ul class="jitrow row-fluid nodrop">
