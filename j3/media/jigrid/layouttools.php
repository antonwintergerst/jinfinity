<?php
/**
 * @version     $Id: layouttools.php 026 2015-08-09 21:38:00Z Anton Wintergerst $
...
class JiGridLayoutTools {
    ...
    /** 
     * Example function to use with findImages
     *
     * Assumes:
     * $this->item as the article object
     * $link has been set already
     * $JiLayoutTools has been set already and is in scope
     */
    function foundImage($i, $image, $replacer, &$text) {
    	// create thumbnail
    	$thumbnail = $JiLayoutTools->createThumbnail($image->path, array('context'=>'grid', 'resize'=>1, 'width'=>200, 'height'=>200, 'fill'=>1, 'cache'=>1));
    	
    	// build replacement HTML
    	ob_start(); ?>
		<a class="grid-image" href="<?php echo $link; ?>" title="<?php echo $this->item->title; ?>">
			<img src="<?php echo $thumbnail->path; ?>"<?php echo $JiLayoutTools->getHTMLAttribs($image->imageattribs, array('class')); ?>/>
		</a>
    	<?php $html = ob_get_clean();
    	
    	// replace original
    	$text = preg_replace($replacer, $html, $text);
    }
    
    /**
     * Example use of findImages
     * Requires foundImage function
     */
    $JiLayoutTools->findImages($item, 'text', array('removeimages'=>0, 'maskclass'=>'jiimg', 'maskmode'=>'exclude', 'limit'=>1), 'foundImage');
     ...
     * @param mixed $params
     */
    public function findImages(&$item, $textvar='text', $params=null, $callback)
    {
            ...
            foreach($matches as $match) {
                ...
                if($params->get('preservelinkattribs', 1)==1) {
                    ...
                    if(isset($linkmatches[0][0])) {
                        ...
                        $replacer = '#<a\s(.*)>(.*)'.$imgtag.'(.*)<\/a>#siU';
                    }

                    $image->linkattribs = $linkattribs;
                }
                
                call_user_func_array($callback, array($i, $image, $replacer, $text));
                $i++;
            }
        }
        $item->{$textvar} = $text;
