<?php
/**
 * @version     $Id: rulesextended.php 050 2014-12-08 11:05:00Z Anton Wintergerst $
...
jiimport('JiAccess');
class JFormFieldRulesExtended extends JFormFieldRules {
    public $type = 'RulesExtended';
    ...
    protected function getInput()
    {
        ...
        foreach ($this->element->children() as $el)
        {
            ...
            {
                // #Jinfinity
                $action = (object) array('name' => (string) $el['name'], 'title' => (string) $el['title'],
                    'description' => (string) $el['description'], 'default'=>(string)$el['default']);
                $actions[] = $action;
                // set default parent value
                if(isset($action->default) && !isset($data[$action->name][1])) $data[$action->name][1] = (bool) $action->default;
            }
        }
        ...
        // #Jinfinity
        $assetRules = JiAccess::getAssetRules($data);
        ...
        foreach ($groups as $group)
        {
            ...
            foreach ($actions as $action)
            {
                ...
                // #Jinfinity
                $inheritedRule = JiAccess::checkGroup($group->value, $action->name, $assetId);
                ...
                if ($canCalculateSettings)
                {
                    ...
                    // #Jinfinity allow permission control over super users
                    if (true || JAccess::checkGroup($group->value, 'core.admin', $assetId) !== true)
                    {
