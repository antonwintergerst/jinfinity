<?php
/**
 * @version     $Id: multiselect.php 093 2014-12-24 10:17:00Z Anton Wintergerst $
...
class JiCustomFieldMultiSelect extends JiCustomField {
    /**
    ...
    public function renderInputScript() {
        ...
        <script type="text/javascript">
            (function(jQuery){
                var JiFieldMultiSelect = function(container, data)
                {
                    var self = this;
                    // Set Default Data
                    this.data = {
                ...
                jQuery.fn.jifieldmultiselect = function(data) {
                    var element = jQuery(this);
                    if(element.data('jifieldmultiselect')) return element.data('jifieldmultiselect');
                    var jifieldmultiselect = new JiFieldMultiSelect(this, data);
                    element.data('jifieldmultiselect', jifieldmultiselect);
                    return jifieldmultiselect;
                };
            })(jQuery);
        </script>
    ...
    public function renderInput() {
        ...
        <div class="select input">
            <select id="<?php echo $this->get('inputid'); ?>" name="<?php echo $this->get('inputname'); ?>[value][]" data-placeholder="<?php echo $this->get('title'); ?>" class="chzn-select" multiple="multiple">
    ...
    public function renderOptions() {
        ...
        // get values for item
        $itemvalues = $this->get('value', 'array');
        foreach((array)$params->get('options') as $value=>$label) {
            ...
            // add html option
            $selected = (in_array($value, $itemvalues))? ' selected="selected"':'';
    ...
    public function renderInputParams() {
        ...
        <div class="jitable optionstable">
            <ul class="jitrow row-fluid nodrop">
                <li class="jitd span12 header"><?php echo JText::_('JICUSTOMFIELDS_MULTISELECT').' '.JText::_('JICUSTOMFIELDS_FIELDPARAMS'); ?></li>
            </ul>
            <ul class="jitrow row-fluid nodrop">
    ...
    public function renderOutput() {
        ...
        // get value for item
        $itemvalues = $this->get('value');
        if(is_object($itemvalues) || is_array($itemvalues)) {
            $itemvalues = (array) $itemvalues;
        } elseif(strpos($itemvalues, ',')!==false) {
            $itemvalues = explode(',', $itemvalues);
        }
        if(!is_array($itemvalues)) $itemvalues = array($itemvalues);
        ...
        foreach($itemvalues as $itemvalue) {
            ...
            if(($itemvalue==null || !isset($options[$itemvalue])) && $params->get('hideempty', '0')==1) continue;

            if(isset($options[$itemvalue])) {
                $class = 'value value-'.htmlspecialchars($itemvalue);
                if($i==0) $class.= ' first';
                if($i==count($itemvalues)-1) $class.= ' last';
                ...
                if($params->get('linkedvalues', 1)==1) {
                    $html = '<a class="jifieldlink '.$class.'" href="'.JRoute::_($link.'&fs['.$this->id.']='.htmlspecialchars($itemvalue)).'" title="View more articles like '.$label.'">'.$valuehtml.'</a>';
                } else {
                    $html.= $valuehtml;
                }
                $i++;
        ...
        // skip/hide empty
        if(empty($html) && $params->get('hideempty', '0')==1) return $html;

        // continue building HTML string
        if($params->get('showlabel', 1)==1) $html = $this->renderLabel().$html;
        $html = $this->get('prefix', '').$html;
