...
/*
 * @version     $Id: view.html.php 021 2014-11-20 11:06:00Z Anton Wintergerst $
 ...
 */
#Jinfinity
class JiCustomFieldsViewCategory extends JViewCategory
{
    /**
    ...
    public function display($tpl = null)
    {
        ...
        if($layout=='default') {
            // default com_jicustomfields output
            parent::addTemplatePath(JPATH_SITE.'/components/com_jicustomfields/views/'.$view.'/tmpl');
            $template = $app->getTemplate();
        } else {
            // default com_content output
            parent::addTemplatePath(JPATH_SITE.'/components/com_content/views/'.$view.'/tmpl');
            $template = $app->getTemplate();
        }
        // template override for com_content
        parent::addTemplatePath(JPATH_THEMES.'/'.$template.'/html/com_content/'.$view);
        // template override for com_jicustomfields
        parent::addTemplatePath(JPATH_THEMES.'/'.$template.'/html/com_jicustomfields/'.$view);

        // #Jinfinity
        $state		= $this->get('State');
        $this->params = $state->params;
        $model = JModelLegacy::getInstance('Category', 'JiCustomFieldsModel', array('ignore_request' => true));
        $this->fields = $model->getFields($this->params->get('show_fields', array(0)), 1);
