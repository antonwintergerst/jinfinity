<?php
/**
 * @version     $Id: import.php 265 2015-01-04 12:17:00Z Anton Wintergerst $
 * @package     Jinfinity Migrator for Joomla 1.7+
...
class JiMigratorModelImport extends JiModel
{
    ...
    function upload() {
        ...
        if(file_exists($zipfile)) {
            ...
            if($haserrors===true) {
                // assume this is an async extraction
                $this->completeAsync('Archive Extracted Successfully!');
            }
        } else {
            $this->completeAsync('Upload Failed!', false);
        }

        if(!$haserrors) {
            $this->completeSync();
        } else {
            $this->completeSync('Upload Failed!', false);
        }
        return;
    }

    public function completeSync($msg='Migration Uploaded Successfully!', $success=true)
    {
    ...
    public function completeAsync($msg='Import Complete!', $success=true)
    {
        $this->completeLog($msg, $success);
    ...
    function cleanup() {
        ...
        $jinput = $app->input;
        if($jinput->get('resetglobalvars', false, 'raw')) {
    ...
    function import() {
        ...
        $params = $this->getParams();
        $processors = array();
        $enabledProcessors = $jinput->get('importers', null, 'raw');
        ...
        $params->set('processall', $jinput->get('processall', false, 'raw'));
        $params->set('dobackup', $jinput->get('dobackup', false, 'raw'));
        $params->set('jifields', $jinput->get('jifields', null, 'raw'));
    ...
    function doimport() {
        ...
        if($this->totalprocessors>0) {
            ...
            $this->completeAsync('ERROR: No processors selected to execute!', false);
        }
        $this->completeSync('Import Completed');
        return;
    }
    ...
    function processor() {
        ...
        if(isset($this->processors[$this->currentprocessor])) {
            ...
            $this->completeAsync('ERROR: Processor was not found or is missing!', false);
        }
    }
    ...
    public function fatal_handler() {
        ...
        switch ($errno) {
            ...
            case E_USER_ERROR:
                ...
                $this->completeAsync('Critical Error!', false);
                break;
