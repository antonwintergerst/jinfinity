<?php
/**
 * @version     $Id: action.php 014 2013-11-13 11:49:00Z Anton Wintergerst $
...
class JiFormsModelAction extends JModelAdmin
{
    /**
     ...
     */
    public function getTable($type = 'Actions', $prefix = 'JiFormsTable', $config = array())
    {
    ...
    public function getItem($pk = null)
    {
        if ($item = parent::getItem($pk)) {
            ...
            if(isset($item->alias)) {
                ...
                $path = JPATH_SITE.DS.'components'.DS.'com_jiforms'.DS.'views'.DS.'action'.DS.'tmpl'.DS.$item->alias.'.php';
                // First check for template overrides
                $stylepath = JPATH_THEMES.DS.$app->getTemplate().DS.'html'.DS.'com_jiforms'.DS.'action'.DS.$item->alias.'.php';
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_jiforms.action', 'action', array('control' => 'jform', 'load_data' => $loadData));
        if (empty($form)) {
            return false;
        }
    ...
    protected function loadFormData()
    {
        ...
        $app  = JFactory::getApplication();
        $data = $app->getUserState('com_jiforms.edit.action.data', array());
        ...
        if (empty($data)) {
            ...
            if($this->getState('action.id') == 0) {
                $data->set('context', $app->input->get('context', $app->getUserState('com_jiforms.actions.filter.context')));
            }
        }
    ...
    public function save($data)
    {
        ...
        try
        {
            ...
            if(isset($table->content) && isset($table->alias)) {
                $fp = fopen(JPATH_SITE.DS.'components'.DS.'com_jiforms'.DS.'views'.DS.'action'.DS.'tmpl'.DS.$table->alias.'.php', "w");
