<?php
/**
 * @version     $Id: form.php 023 2014-11-05 11:47:00Z Anton Wintergerst $
...
class JiFormsModelForm extends JModelAdmin
{
    /**
    ...
    function preview() {
        ...
        $form->content = $jinput->get('content', '', 'raw');
        $form->data = $this->getFields($form);
     ...
     */
    public function getTable($type = 'Forms', $prefix = 'JiFormsTable', $config = array())
    {
    ...
    public function getItem($pk = null)
    {
        if ($item = parent::getItem($pk)) {
            ...
            if(isset($item->alias)) {
                ...
                $path = JPATH_SITE.DS.'components'.DS.'com_jiforms'.DS.'views'.DS.'form'.DS.'tmpl'.DS.$item->alias.'.php';
                // First check for template overrides
                $stylepath = JPATH_THEMES.DS.$app->getTemplate().DS.'html'.DS.'com_jiforms'.DS.'form'.DS.$item->alias.'.php';
                if(file_exists($stylepath)) {
                    $item->content = file_get_contents($stylepath);
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_jiforms.form', 'form', array('control' => 'jform', 'load_data' => $loadData));
        if (empty($form)) {
            return false;
        }
        $jinput = JFactory::getApplication()->input;
    ...
    protected function loadFormData()
    {
        ...
        $app  = JFactory::getApplication();
        $data = $app->getUserState('com_jiforms.edit.form.data', array());
        ...
        if (empty($data)) {
            ...
            if($this->getState('form.id') == 0) {
                $data->set('context', $app->input->get('context', $app->getUserState('com_jiforms.forms.filter.context')));
            }
        }
    ...
    public function save($data)
    {
        ...
        try
        {
            ...
            if(isset($table->content) && isset($table->alias)) {
                $fp = fopen(JPATH_SITE.DS.'components'.DS.'com_jiforms'.DS.'views'.DS.'form'.DS.'tmpl'.DS.$table->alias.'.php', "w");
                fwrite($fp, $table->content);
                fclose($fp);
            }
