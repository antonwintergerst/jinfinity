/**
 * @version     $Id: jquery.jimediabrowser.js 081 2014-12-16 20:18:00Z Anton Wintergerst $
    ...
    var JiMediaBrowser = function(container, options)
    {
        ...
        this.buildlist = function() {
            if(this.data.items!=null && !this.isbuilding) {
                ...
                var mbbox = jQuery(document.createElement('ul')).attr({'class':'mbbox'});
                // Create head
                var mbhead = jQuery(document.createElement('li')).attr({'class':'mbhead'});

                this.createTitleBar(mbhead);

                if(self.style=='list') {
                    // Create column titles
                    var mbrow = jQuery(document.createElement('ul')).attr({'class':'mbrow columnnames'});
                    // Column 1
                    var mbcol = jQuery(document.createElement('li')).attr({'class': 'col1 span1 mimeicon'});
                    jQuery(mbrow).append(mbcol);
                    // Column 2
                    mbcol = jQuery(document.createElement('li')).attr({'class': 'col2 span8 name'}).html('Name');
                    jQuery(mbrow).append(mbcol);
                    // Column 3
                    mbcol = jQuery(document.createElement('li')).attr({'class': 'col3 span2 size'}).html('Size');
                    jQuery(mbrow).append(mbcol);
                    // Column 4
                    mbcol = jQuery(document.createElement('li')).attr({'class': 'col4 span1 action'}).html('Action');
                    jQuery(mbrow).append(mbcol);

                    jQuery(mbhead).append(mbrow);
                }
                jQuery(mbbox).append(mbhead);
