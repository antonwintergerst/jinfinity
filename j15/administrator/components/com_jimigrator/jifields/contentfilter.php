<?php
/**
 * @version     $Id: contentfilter.php 097 2014-12-15 10:57:00Z Anton Wintergerst $
...
class JiMigratorFieldContentFilter extends JiMigratorField {
    function getFilter() {
        $listfilterhelper = new JiListFilterHelper();
        if(version_compare( JVERSION, '1.6.0', 'ge' )) {
            $includepath = JPATH_SITE.'/administrator/components/com_jimigrator/helpers/j25contentfilter.php';
        } else {
            $includepath = JPATH_SITE.'/administrator/components/com_jimigrator/helpers/j15contentfilter.php';
        }
        $listfilter = $listfilterhelper->loadType('content', $includepath);
        return $listfilter;
    }
            ...
            if(typeof jQuery!='undefined') {
                jQuery(document).ready(function() {
                    jQuery({
                        ...
                        url:'<?php echo JURI::root().'administrator/index.php?option=com_jimigrator&api=json&view=listfilter&filtertype=content&scope='.$this->get('id').'&name='.$this->get('name'); ?>',
